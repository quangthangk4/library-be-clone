...
public class SearchPublicationsUseCaseImpl implements SearchPublicationsUseCase {
    ...
    public PageResponse<PublicationResponse> execute(SearchPublicationRequest request) {
        log.info("Searching publications with criteria: {}", request);

        Pageable pageable = PageRequest.of(request.page(), request.size());

        // Simple title search for now
        // Note: A full implementation would use Specification pattern or QueryDSL
        // for complex multi-criteria search
        Page<Publication> publicationPage;
        if (request.title() != null && !request.title().isBlank()) {
            publicationPage = publicationRepository.searchByTitle(request.title(), pageable);
        } else {
